...
export const LoginForm = ({ error, onLogin }) => {
  ...
  return (
    <form onSubmit={handleSubmit((values) => onLogin(values))} noValidate className="space-y-5">
      ...
      <div>
        <label htmlFor="password" className="mb-2 flex items-center justify-between text-sm font-semibold text-slate-700 dark:text-slate-200">
          <span>Password</span>
      ...
      <div>
        <label htmlFor="role" className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Role</label>
        <Controller name="role" control={control} render={({ field }) => (
          <select id="role" aria-invalid={Boolean(errors.role)} className={`w-full rounded-xl border bg-white dark:bg-slate-800 px-3.5 py-3 text-sm text-slate-800 dark:text-white outline-none transition focus:ring-2 focus:ring-orange-100 ${errors.role ? fieldErrorClass : 'border-slate-200 dark:border-slate-700 focus:border-orange-500'}`} {...field}>
      ...
      <div>
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3.5 py-3 text-sm text-slate-600 dark:text-slate-300">
