                ...
                {isSafetyOfficer ? (
                  <>
                    <div className="pt-2 flex flex-col gap-3">
                      <div>
                        <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">License Expiry Date</label>
                        <input type="date" value={editedFields.expiryDate} onChange={(e) => setEditedFields({...editedFields, expiryDate: e.target.value})} className="w-full text-sm font-semibold text-slate-600 p-2 border border-slate-200 rounded-md bg-slate-50 focus:ring-1 focus:ring-blue-500" />
                      </div>
                      <div>
                        <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">Contact Phone</label>
                        <input type="text" value={editedFields.phone} onChange={(e) => setEditedFields({...editedFields, phone: e.target.value})} className="w-full text-sm font-semibold text-slate-600 p-2 border border-slate-200 rounded-md bg-slate-50 focus:ring-1 focus:ring-blue-500" />
                      </div>
                    </div>
                  </>
                ) : (
                ...
                <div className="space-y-1.5 flex-1">
                  ...
                  {isSafetyOfficer && (
                    <div className="mt-2 pt-2 border-t border-slate-100 flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        <label className="text-[10px] font-bold text-slate-400 uppercase">Override Score:</label>
          ...
          {isSafetyOfficer && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition">
                    <div>
                      <p className="text-xs font-bold text-slate-700">Driving License</p>
                      <p className="text-[10px] text-slate-400">Required</p>
                    </div>
                    <button onClick={() => alert('Mock upload dialog opened')} className="text-xs bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-1.5 px-3 rounded">Upload</button>
                  </div>
                  <div className="flex justify-between items-center p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition">
                    <div>
