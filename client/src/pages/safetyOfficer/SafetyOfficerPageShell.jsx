    ...
    {(title || subtitle || actions) && (
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          {subtitle && <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">{subtitle}</p>}
        </div>
        {actions}
      </div>
    )}
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
    {items.map((item) => (
      <div key={item.label} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
        <p className="text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-semibold">{item.label}</p>
        <div className="mt-3 text-3xl font-bold text-slate-800 dark:text-white">{item.value}</div>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.note}</p>
      </div>
    ))}
  </div>
);
  ...
  <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
    <table className="min-w-full divide-y divide-slate-200">
      <thead className="bg-slate-50 dark:bg-slate-900">
        <tr>
          {columns.map((column) => (
            <th key={column.key} className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400 dark:text-slate-500">
              {column.label}
            </th>
          ))}
        </tr>
      ...
      <tbody className="divide-y divide-slate-200 bg-white dark:bg-slate-800">
        {rows.map((row) => (
          <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900/80 transition-colors">
            {columns.map((column) => (
