...
export const RegisterForm = ({ onRegister }) => {
  ...
  return <form onSubmit={handleSubmit((values) => onRegister(values))} noValidate className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2"><div><label htmlFor="password" className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Password</label><PasswordInput id="password" autoComplete="new-password" placeholder="Create a password" error={errors.password?.message} {...passwordField} /></div><div><label htmlFor="confirmPassword" className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Confirm Password</label><PasswordInput id="confirmPassword" autoComplete="new-password" placeholder="Confirm your password" error={errors.confirmPassword?.message} {...confirmPasswordField} /></div></div>
    <div className="grid gap-5 sm:grid-cols-2"><div><label htmlFor="phoneNumber" className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Phone Number</label><Input id="phoneNumber" type="tel" inputMode="numeric" maxLength={10} placeholder="10 digit phone number" aria-invalid={Boolean(errors.phoneNumber)} className={errors.phoneNumber ? fieldErrorClass : ''} {...phoneNumberField} /><FieldError message={errors.phoneNumber?.message} /></div><div><label htmlFor="role" className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-200">Role</label><Controller name="role" control={control} render={({ field }) => <select id="role" aria-invalid={Boolean(errors.role)} className={`w-full rounded-xl border bg-white dark:bg-slate-800 px-3.5 py-3 text-sm text-slate-800 dark:text-white outline-none transition focus:ring-2 focus:ring-amber-100 ${errors.role ? fieldErrorClass : 'border-slate-200 dark:border-slate-700 focus:border-amber-500'}`} {...field}><option value="">Select a role</option>{roleOptions.map((role) => <option key={role} value={role}>{role}</option>)}</select>} /><FieldError message={errors.role?.message} /></div></div>
    <div><label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3.5 py-3 text-sm text-slate-600 dark:text-slate-300"><Controller name="terms" control={control} render={({ field }) => <input type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 text-amber-500 focus:ring-amber-500" aria-invalid={Boolean(errors.terms)} checked={field.value} onChange={(event) => field.onChange(event.target.checked)} onBlur={field.onBlur} ref={field.ref} />} /><span>I agree to the Terms &amp; Conditions.</span></label><FieldError message={errors.terms?.message} /></div>
