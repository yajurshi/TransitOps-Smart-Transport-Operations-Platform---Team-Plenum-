      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 text-xs uppercase tracking-widest mb-4">Trip Timeline & Progress</h3>
            
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative">
              {timelineSteps.map((step, idx) =>
              <React.Fragment key={idx}>
                  <div className="flex items-center gap-3 z-10 bg-white px-2">
                  ...
                  'bg-slate-50 border-slate-200 text-slate-400'}`
                    ...
                    <div>
                      <span className={`block text-xs font-bold ${step.status === 'completed' ? 'text-slate-800' : 'text-slate-400'}`}>{step.label}</span>
                      <span className="text-[10px] text-slate-400 font-semibold">{step.status === 'completed' ? 'Timestamp Recorded' : 'Awaiting status'}</span>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-5">
              <div className="border-b border-slate-100 pb-3">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Manifest Specifications</h3>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Source</span>
                  <span className="text-sm font-semibold text-slate-700 block mt-1">{trip.source}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Destination</span>
                  <span className="text-sm font-semibold text-slate-700 block mt-1">{trip.destination}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Cargo weight</span>
                  <span className="text-sm font-semibold text-slate-700 block mt-1">{trip.cargoWeight.toLocaleString()} kg</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Distance</span>
                  <span className="text-sm font-semibold text-slate-700 block mt-1">{trip.distance.toLocaleString()} km</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">ETA</span>
                  <span className="font-mono text-sm font-bold text-slate-700 block mt-1">{trip.eta}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Trip Region</span>
                  <span className="text-sm font-semibold text-slate-700 block mt-1">{trip.region} Region</span>
                </div>
              </div>

              {trip.notes &&
              <div className="pt-2 border-t border-slate-100">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Notes</span>
                  <p className="text-xs text-slate-500 font-medium mt-1 leading-normal">{trip.notes}</p>
                </div>
              }
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-5">
              <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Allocated Vehicle</h3>
                {assignedVehicle &&
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="text-sm font-semibold text-slate-700 block mt-1">{assignedVehicle.name}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Registration</span>
                    <span className="font-mono text-sm font-bold text-slate-700 block mt-1">{assignedVehicle.reg}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Payload Capacity</span>
                    <span className="text-sm font-semibold text-slate-500 block mt-1">{assignedVehicle.capacity.toLocaleString()} kg</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Current Odometer</span>
                    <span className="font-mono text-sm font-semibold text-slate-500 block mt-1">{assignedVehicle.odometer.toLocaleString()} km</span>
                  </div>
                </div> :

              <p className="text-xs text-slate-400 font-medium">No vehicle information found.</p>
              }
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-5">
              <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Assigned Operator</h3>
                {assignedDriver &&
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Operator Name</span>
                    <span className="text-sm font-semibold text-slate-700 block mt-1">{assignedDriver.name}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">License Number</span>
                    <span className="font-mono text-sm font-bold text-slate-700 block mt-1">{assignedDriver.licenseNumber}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">License Class</span>
                    <span className="text-sm font-semibold text-slate-500 block mt-1">{assignedDriver.licenseCategory}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Operator Status</span>
                    <span className="text-sm font-semibold text-slate-500 block mt-1">{assignedDriver.status}</span>
                  </div>
                </div> :

              <p className="text-xs text-slate-400 font-medium">No driver information found.</p>
              }
            </div>
